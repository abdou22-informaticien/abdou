...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["enregistrer"])) {
        if (!empty($code) && !empty($filiere)) {
            // Vérifier si le code ou la filière existe déjà
            $check_sql = "SELECT * FROM filiere WHERE code = ? OR filiere = ?";
            $stmt_check = $conn->prepare($check_sql);
            $stmt_check->bind_param("ss", $code, $filiere);
            $stmt_check->execute();
            $result_check = $stmt_check->get_result();

            if ($result_check->num_rows > 0) {
                echo "Le code ou la filière existe déjà.";
            } else {
                // Utiliser des requêtes préparées pour éviter les injections SQL
                $stmt = $conn->prepare("INSERT INTO filiere (code, filiere) VALUES (?, ?)");
                $stmt->bind_param("ss", $code, $filiere);
    ...
    if (isset($_POST["afficherliste"])) {
        $sql_select = "SELECT * FROM filiere";
        ...
        if ($result_select->num_rows > 0) {
            echo "<h2>Liste des filières :</h2>";
            echo "<table>";
            echo "<tr><th>Code</th><th>Filière</th></tr>";
